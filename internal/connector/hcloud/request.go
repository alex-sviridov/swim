...
type ProvisionRequest struct {
	ServerType       string
	FirewallID       string // Hetzner Cloud Firewall ID
	ImageID          string
	Location         string
	SSHKey           string // SSH key name or ID
	WebUsername      string
	LabID            int
...
func UnmarshalAndValidate(payload string) (*ProvisionRequest, error) {
	...
	if req.ServerType == "" {
		req.ServerType = os.Getenv("HCLOUD_DEFAULT_SERVER_TYPE")
		if req.ServerType == "" {
			missing = append(missing, "ServerType")
		}
	}
	if req.FirewallID == "" {
		req.FirewallID = os.Getenv("HCLOUD_DEFAULT_FIREWALL")
		if req.FirewallID == "" {
			missing = append(missing, "FirewallID")
		}
	}
	if req.ImageID == "" {
		req.ImageID = os.Getenv("HCLOUD_DEFAULT_IMAGE")
		if req.ImageID == "" {
			missing = append(missing, "ImageID")
		}
	}
	if req.Location == "" {
		req.Location = os.Getenv("HCLOUD_DEFAULT_LOCATION")
		if req.Location == "" {
			missing = append(missing, "Location")
		}
	}
	if req.SSHKey == "" {
		req.SSHKey = os.Getenv("HCLOUD_DEFAULT_SSH_KEY")
		if req.SSHKey == "" {
